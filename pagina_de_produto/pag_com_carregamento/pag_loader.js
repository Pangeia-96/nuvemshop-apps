(function () {
  ...
  /* ============================================================
     NIKE-STYLE LOADER & TRANSITION MASK
     ============================================================ */
  (function () {
    // 1. BLOQUEIO SÍNCRONO TOTAL (Executa antes de qualquer injeção)
    const path = window.location.pathname;
    const isHome = path === "/" || path === "/index.html" || document.body?.classList.contains('template-home');
    const isProductPage = path.includes("/produtos/") || document.body?.classList.contains('template-product');

    // Se estiver na Home ou não for produto, mata o script do loader IMEDIATAMENTE
    if (isHome || !isProductPage) return;

    // Evitar execução duplicada ou bypass via search query
    if (window.__P96LoaderLoaded || window.location.search.includes('no-loader')) return;
    window.__P96LoaderLoaded = true;

    // 2. CRIAÇÃO DOS ESTILOS
    const style = document.createElement('style');
    style.id = 'p96-loader-styles';
    style.textContent = `
            #p96-loader-overlay {
            ...
            .p96-loader-content { text-align: center; }
            .p96-loader-spinner {
                width: 42px; height: 42px; border: 3px solid rgba(212, 22, 200, 0.05); border-top: 3px solid #d416c8;
                border-radius: 50%; animation: p96-spin 0.8s cubic-bezier(0.4, 0, 0.2, 1) infinite; margin: 0 auto;
                box-shadow: 0 0 15px rgba(212, 22, 200, 0.05);
            }
            @keyframes p96-spin { to { transform: rotate(360deg); } }
            #p96-loader-bar {
                position: fixed; top: 0; left: 0; height: 3px; background: #d416c8; z-index: 2147483647;
                width: 0%; transition: width 0.3s ease; box-shadow: 0 0 8px rgba(212, 22, 200, 0.2);
            }
            .p96-loader-hidden { opacity: 0 !important; visibility: hidden !important; pointer-events: none !important; }
        `;

    // 3. CRIAÇÃO DOS ELEMENTOS
    const overlay = document.createElement('div');
    overlay.id = 'p96-loader-overlay';
    overlay.innerHTML = '<div class="p96-loader-content"><div class="p96-loader-spinner"></div></div>';
    const bar = document.createElement('div');
    bar.id = 'p96-loader-bar';

    // 4. INJEÇÃO ULTRA-RÁPIDA
    const inject = () => {
      const head = document.head || document.documentElement;
      const target = document.body || document.documentElement;

      if (head && !document.getElementById('p96-loader-styles')) head.appendChild(style);

      if (target && !document.getElementById('p96-loader-overlay')) {
        // Tripla checagem antes de inserir no DOM
        const currentPath = window.location.pathname;
        if (currentPath === "/" || document.body?.classList.contains('template-home')) return;
    ...
    // Tentar injetar agora e também no DOMContentLoaded (para redundância)
    inject();
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', inject);
    }

    // 5. API DE CONTROLE DO LOADER
    window.__p96Loader = {
      done: { accordion: false, gallery: false, header: false },
      update: function (comp) {
        if (comp) this.done[comp] = true;
        let p = 0;
        if (this.done.accordion) p += 35;
        if (this.done.gallery) p += 35;
        if (this.done.header) p += 30;
        const b = document.getElementById('p96-loader-bar');
        if (b) b.style.width = p + '%';
        if (p >= 100) this.hide();
      },
      hide: function () {
        const o = document.getElementById('p96-loader-overlay');
        const b = document.getElementById('p96-loader-bar');
        if (o) o.classList.add('p96-loader-hidden');
        if (b) b.classList.add('p96-loader-hidden');
        setTimeout(() => { o?.remove(); b?.remove(); }, 600);
      }
    };

    // Failsafe: se nada acontecer em 4 segundos, remove o loader
    setTimeout(() => window.__p96Loader?.update('header'), 3500);
    setTimeout(() => window.__p96Loader?.hide(), 4500);
  })();
  ...
  function init() {
    ...
    // Notificar Loader
    if (window.__p96Loader) window.__p96Loader.update('accordion');
  }
...
(async function () {
  ...
  function start() {
    ...
    waitForSwiper(function (swiperEl) {
      ...
      // Notificar Loader
      if (window.__p96Loader) window.__p96Loader.update('gallery');
    });
  }
...
(function () {
  ...
  function init() {
    ...
    // Notificar Loader (Header é rápido)
    if (window.__p96Loader) window.__p96Loader.update('header');
  }
